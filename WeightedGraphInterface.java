// Interface for a class that implements a directed 
...
public interface WeightedGraphInterface {
    ...
    // Postcondition:  Return value = (this graph is     
    // empty)
    ...
    // Effect: Adds an edge with the specified weight 
    // from fromVertex to toVertex.
    // Precondition: fromVertex and toVertex are in 
    // V(graph).
    // Postcondition:  (fromVertex, toVertex) is in 
    // E(graph) with the specified weight.

    public int weightIs(Object fromVertex, Object toVertex);
    // Effect: Determines the weight of the edge from 
    // fromVertex to toVertex.
    // Precondition: fromVertex and toVertex are in 
    // V(graph).
    // Postcondition:  if edge exists, Return value = 
    // weight of edge from fromVertex to toVertex)
    // otherwise return value = (special “null-edge” 
    // value)

    public QueueInterface getToVertices(Object vertex);
    // Effect: Returns a queue of the vertices that are 
    // adjacent from vertex.
    // Precondition:   vertex is in V(graph).
    // Postcondition:  returns a queue containing all the 
    // vertices that are adjacent from vertex.
    ...
    // Precondition:   vertex is in V(graph)
    // Postcondition:  return value = (vertex is marked 
    // true)
